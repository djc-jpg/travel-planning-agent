"""500-request concurrency isolation pressure test."""
...
def test_concurrency_500_isolation_and_success_rate():
    ctx = AppContext(
    ...
    def _run_case(i: int):
        session_id = f"stress500_{i:03d}"
        cache_key = f"cache500_{i:03d}"
        result = plan_trip(
            TripRequest(
                message=f"杭州1天 并发{i}",
                session_id=session_id,
    ...
    with ThreadPoolExecutor(max_workers=120) as pool:
        rows = list(pool.map(_run_case, range(500)))
